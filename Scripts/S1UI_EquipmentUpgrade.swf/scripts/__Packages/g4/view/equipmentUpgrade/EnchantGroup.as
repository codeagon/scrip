class g4.view.equipmentUpgrade.EnchantGroup extends gfx.core.UIComponent
{
   ...
   function onLoad()
   {
      ...
      this.mResultSuccess.mText.textField.text = lib.util.UIString.getUIString("$155010");
      this.mResultDestroy.mText.textField.text = lib.util.UIString.getUIString("$710014");
      this.tGuide.htmlText = lib.util.UIString.getUIString("$155004");
      ...
      this.clearAll();
      this.mSlotEffect.hitTestDisable = true;
   ...
   {
      this.dispatchEvent({type:"ToGame_Enchant_SelectedMaterialSlot",index:$index});
   }
   function confirmHandlerClick(event)
   {
      this.dispatchEvent({type:"ToGame_EquipmentUpgrade_Click",index:0});
   }
   function clearAll()
   {
   ...
   {
      this.tItemName.htmlText = "";
      this.tCurrent.htmlText = "";
      this.tNext.text = "";
      this.tItemLevel.text = "";
      this.tGuide._visible = true;
      this.mItemName._visible = false;
      this.mEnchantCurrent._visible = false;
      this.mEnchantNext._visible = false;
      ...
      this.mSlotEffect._visible = true;
      this.mSlotEffect.gotoAndPlay(2);
   ...
   function setTargetItem($itemID, $img, $grade, $name, $enchant, $bMax, $currentLevel, $maxLevel, $currentInfo, $nextInfo)
   {
      this.clearTargetItem();
      ...
      $bMax = Number($bMax);
      var _loc7_ = lib.util.UIString.getUIString("$155008","currentLevel",$currentLevel,"maxLevel",$maxLevel);
      this.tItemLevel.htmlText = _loc7_;
      this.tItemName.htmlText = $name;
      this.tItemName._visible = true;
      this.mItemName._visible = true;
      this.EquipmentUpgrade__0.id = $itemID;
      var _loc5_ = $bMax != 1?$enchant + 1:$enchant;
      this.mEnchantCurrent.gotoAndStop($enchant + 1);
      this.mEnchantNext.gotoAndStop(_loc5_ + 1);
      this.mEnchantCurrent._visible = true;
      this.mEnchantNext._visible = true;
      this.tCurrent.htmlText = $currentInfo;
      this.tNext.htmlText = $nextInfo;
      this.tCurrent._visible = $bMax != 1;
      this.tNext._visible = !$bMax;
      this.EquipmentUpgrade__0.draw($img,64,64);
      this.EquipmentUpgrade__0.__set__grade($grade);
      lib.manager.ToolTip.add(this.EquipmentUpgrade__0.SLOT,$itemID + "\t0\t0",6);
      this.tGuide._visible = false;
      this.mSlotEffect.gotoAndStop(1);
      this.mSlotEffect._visible = false;
   }
   function setClearTargetItem()
   {
      this.clearAll();
   }
   function setMaterialList($itemParam, $gold, $bEnough, $rateMessage, $tooltipDetail)
   {
      this.clearMaterialList();
      ...
      while(_loc4_ < 8)
      {
         ...
         if(_loc4_ < _loc14_)
         {
            ...
            lib.manager.ToolTip.add(_loc2_.SLOT,_loc8_ + "\t0\t" + _loc7_,6);
         }
         else
         {
   ...
   {
      this.dispatchEvent({type:"ToGame_EquipmentUpgrade_EndStartEffect",index:0});
   }
   function setResult($success)
   {
      this._nResult = $success;
      if($success == 1)
      {
      ...
      else if($success >= 2 && $success <= 5)
      {
         if($success == 2)
         {
            this.mResultDown.mText.textField.text = lib.util.UIString.getUIString("$155011");
         }
         else if($success == 3)
         {
            this.mResultDown.mText.textField.text = lib.util.UIString.getUIString("$155076");
         }
         else if($success == 4)
         {
            this.mResultDown.mText.textField.text = lib.util.UIString.getUIString("$155077");
         }
         else if($success == 5)
         {
            this.mResultDown.mText.textField.text = lib.util.UIString.getUIString("$155078");
         }
   ...
   function resultTitleHandlerFinish()
   {
      ...
      if(!(this._nResult == 1 || this._nResult == 2 || this._nResult == 3))
      {
