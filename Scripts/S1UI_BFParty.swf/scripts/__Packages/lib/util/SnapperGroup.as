class lib.util.SnapperGroup extends lib.gfx.events.EventDispatcher implements mx.utils.Collection
{
   var tickInterval = 1;
   var bletTickness = 3;
   ...
   function addItem(snapper)
   {
      ...
      if(snapper != null && snapper instanceof lib.util.Snapper)
      {
         ...
         snapper.addEventListener(lib.gfx.events.EventTypes.DRAG_BEGIN,this,"dragBeginHandler");
         snapper.addEventListener(lib.gfx.events.EventTypes.DRAG_END,this,"dragEndHandler");
   ...
   function removeItem(snapper)
   {
      ...
      if(_loc3_ > -1)
      {
         snapper.removeEventListener(lib.gfx.events.EventTypes.DRAG_BEGIN,this,"dragBeginHandler");
         snapper.removeEventListener(lib.gfx.events.EventTypes.DRAG_END,this,"dragEndHandler");
   ...
   function dragBeginHandler(event)
   {
      ...
      this.dispatchEvent({type:lib.gfx.events.EventTypes.DRAG_BEGIN,target:event.target});
   }
   function dragEndHandler(event)
   {
      ...
      this.draggigSnapper = null;
      this.unionMove();
      this.dispatchEvent({type:lib.gfx.events.EventTypes.DRAG_END,target:event.target});
   }
