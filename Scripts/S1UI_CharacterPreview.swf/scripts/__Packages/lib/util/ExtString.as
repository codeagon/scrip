class lib.util.ExtString
{
   ...
   static function textCut(textFd, str)
   {
      ...
      var _loc6_ = textFd.getLineMetrics(0);
      ...
      if(!textFd.multiline)
      {
         ...
         lib.Debuger.trace("lineW  " + _loc8_);
         _loc4_ = textFd.getCharIndexAtPoint(_loc8_,_loc7_);
      }
      else
      {
         _loc7_ = _loc6_.ascent + _loc6_.descent + _loc6_.leading;
         _loc9_ = int(textFd._height / _loc7_);
         _loc4_ = textFd.getLineOffset(_loc9_);
      }
      textFd.isTextCut = true;
      if(_loc5_ > _loc4_ && _loc4_ != -1)
      {
         if(_loc5_ > _loc4_ + 2)
         {
            textFd.replaceText(_loc4_ - 2,_loc5_,"…");
            if(arguments[2])
            {
               if(!textFd.html)
               {
