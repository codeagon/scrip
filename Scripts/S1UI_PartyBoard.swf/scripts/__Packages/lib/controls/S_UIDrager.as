class lib.controls.S_UIDrager extends mx.core.UIComponent
{
   ...
   function beginDrag()
   {
      var _loc5_ = lib.info.AlignInfo.uiScale(this.UIName,this.dragTarget_mc);
      var _loc6_ = _root.GetRegisterUIData(this.dragTarget_mc._name);
      var _loc3_ = 1;
      if(_loc6_.ScaleCustom)
      {
         _loc3_ = Number(_loc6_.ScaleCustom) / Number(lib.gamedata.DefaultData.getDefaultUIData("uiscale").param0);
      }
      var _loc4_ = lib.info.AlignInfo.scaleRatio * _loc5_ * _loc3_;
      this.leftLimit = - lib.info.AlignInfo.horizontalSpace + this.adjustLeft;
      this.topLimit = - lib.info.AlignInfo.verticalSpace + this.adjustTop;
      this.rightLimit = Stage.width - lib.info.AlignInfo.horizontalSpace - this._width * _loc4_ + this.adjustRight;
      this.bottomLimit = lib.info.AlignInfo.UI_HEIGHT + lib.info.AlignInfo.verticalSpace - this._height * _loc4_ + this.adjustBottom;
      ...
      lib.Debuger.trace("(라이트)" + this.rightLimit + " = " + (Stage.width - lib.info.AlignInfo.horizontalSpace) + "-" + this._width + " * " + lib.info.AlignInfo.scaleRatio + " * " + _loc5_);
      this.tweendrag();
   }
