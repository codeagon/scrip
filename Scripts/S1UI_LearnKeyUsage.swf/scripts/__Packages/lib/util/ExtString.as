class lib.util.ExtString
{
   ...
   static function textCut(textFd, str)
   {
      ...
      var _loc10_ = textFd.getLineMetrics(0);
      var _loc13_ = _loc10_.ascent + _loc10_.descent + _loc10_.leading;
      var _loc6_ = int(textFd._height / _loc13_);
      var _loc11_ = String(textFd.text).length;
      var _loc8_ = textFd.getLineLength(_loc6_);
      var _loc3_ = 0;
      ...
      while(_loc4_ < _loc6_)
      {
         _loc3_ = _loc3_ + textFd.getLineLength(_loc4_);
         _loc4_ = _loc4_ + 1;
      }
      lib.manager.ToolTip.remove(textFd);
      var _loc7_ = textFd.getLineMetrics(_loc6_ - 1);
      if(_loc8_ > 0)
      {
         if(_loc8_ > 2)
         {
            var _loc5_ = textFd.getCharBoundaries(_loc3_ + 1).width;
            while(_loc7_.width + _loc5_ < textFd._width - 4)
            {
               _loc3_ = _loc3_ + 1;
               _loc5_ = _loc5_ + textFd.getCharBoundaries(_loc3_ + 1).width;
            }
            textFd.replaceText(_loc3_ - 1,_loc11_,"…");
            if(textFd.getLineLength(_loc6_) > 0)
            {
               textFd.replaceText(_loc3_ - 2,_loc3_ + 1,"…");
            }
            if(!textFd.multiline)
            {
         ...
         {
            var _loc12_ = _loc3_ / (_loc3_ + _loc8_);
            textFd.fontScaleFactor = _loc12_;
            if(textFd.html)
            {
   ...
   static function findTag(htmlString, tagStr)
   {
      ...
      while(_loc4_ != -1)
      {
         var _loc6_ = {tagindex:lib.util.ExtString.indexOfTag(htmlString,tagStr,_loc4_)};
         _loc6_.tagendindex = lib.util.ExtString.indexOfTag(htmlString,"/" + tagStr,_loc4_);
         var _loc12_ = htmlString.indexOf("<",_loc4_);
         var _loc10_ = htmlString.indexOf(">",_loc12_);
         ...
         while(_loc2_ != -1)
         {
            ...
            var _loc7_ = lib.util.ExtString.trim(_loc1_.substring(_loc9_,_loc2_));
            var _loc8_ = _loc1_.substring(_loc2_ + 2,_loc3_);
            _loc6_[_loc7_] = _loc8_;
            _loc2_ = _loc1_.indexOf("=\'",_loc3_);
         }
         _loc15_.push(_loc6_);
         _loc4_ = htmlString.indexOf("<" + tagStr + " ",_loc10_);
         if(_loc4_ == -1)
         {
            _loc4_ = htmlString.indexOf("<" + tagStr + ">",_loc10_);
         }
