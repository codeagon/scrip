class lib.util.InteractionGroup
{
   ...
   static function listenerEvnet(_targetMc, interactionStr)
   {
      ...
      var _loc2_ = _targetMc.id.substr(_loc4_ + 1,_loc5_);
      var _loc7_ = _targetMc.id.substr(0,_loc4_ + 1);
      if(interactionStr == "" || interactionStr == undefined)
      {
         if(_global.shift)
         {
            getURL("FSCommand:ToGame_Chat_ClickName",_loc2_ + "\t\t\tSHIFT");
         }
         else if(_global.control)
         {
            getURL("FSCommand:ToGame_Chat_ClickName",_loc2_ + "\t\t\tCTRL");
         }
         else
         {
            getURL("FSCommand:ToGame_Chat_ClickName",_loc2_ + "\t\t\t");
         }
      }
      else
      {
         getURL("FSCommand:" add interactionStr,_loc2_ + "\t\t\t");
      }
   }
