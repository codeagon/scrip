class lib.controls.S_SimpleScrollBar extends MovieClip
{
   ...
   function draw()
   {
      ...
      this.setScrollSize((this.barMc._height - this.barBtn._height) / this._scrollMax);
   ...
   function draging(tgy)
   {
      ...
      this._listener.onScroll(_loc2_,this._scroll);
   }
   ...
   function setWheel(target)
   {
      ...
      {
         if(_this._parent._parent.UIname == "SystemOption")
         {
            lib.Debuger.trace("  onMouseWheel  ::  " + _this + "   ::  " + this.id);
         }
         var _loc4_ = String(scTg).split(".");
         var _loc2_ = _loc4_.length - 1;
         var _loc3_ = String(scrollTarget).split(".").slice(0,_loc2_ + 1).join(".");
         var _loc5_ = String(scrollTarget).split(".").slice(0,_loc2_).join(".");
         if(_loc3_ == String(scTg))
         {
            _this.wheel(delta);
         }
         else if(_loc5_ == String(scTg._parent))
         {
   ...
   {
      lib.Debuger.trace("setScrollPosition    " + int(py / this._rowHeight));
