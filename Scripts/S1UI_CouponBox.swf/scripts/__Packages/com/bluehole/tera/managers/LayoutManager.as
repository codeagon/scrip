class com.bluehole.tera.managers.LayoutManager
{
   ...
   static function saveUIPositon(ui, pivotType, lockState, optionCustomX, optionCustomY)
   {
      ...
      var _loc4_ = com.bluehole.tera.managers.LayoutManager.getOriginCoord(ui);
      var _loc3_ = com.bluehole.tera.managers.LayoutManager.getPivotPoint(ui,pivotType);
      var _loc7_ = optionCustomX == undefined?(ui._x + _loc3_.x - _loc4_.x) * 10000 / _loc5_._xscale / Stage.visibleRect.width:optionCustomX;
      var _loc6_ = optionCustomY == undefined?(ui._y + _loc3_.y - _loc4_.y) * 10000 / _loc5_._yscale / Stage.visibleRect.height:optionCustomY;
      _root.ToGame_SaveUIPosition(lockState,ui._name,_loc7_,_loc6_);
   ...
   static function openingAni2(ui)
   {
      ui._alpha = 0;
      gs.TweenMax.killTweensOf(ui,false);
      gs.TweenMax.to(ui,0.3,{_alpha:100,ease:gs.easing.Linear.easeOut});
   }
   ...
   function addAutoPositionActor(actor)
   {
      ...
      this.reflowAutoLayoutUIs(com.bluehole.tera.managers.LayoutManager.LAYOUT_REFLOW_MODE_UNMANAGED_UI);
   }
   ...
   {
      var _loc2_ = this.requestedAutoLayoutModes.length;
      while(_loc2_ >= 0)
      {
         if(this.requestedAutoLayoutModes[_loc2_] == mode)
         {
            this.requestedAutoLayoutModes.splice(_loc2_,1);
         }
         _loc2_ = _loc2_ - 1;
      }
      this.requestedAutoLayoutModes.push(mode);
   ...
   function resetUIPosition()
   {
      ...
      this.reflowAutoLayoutUIs(com.bluehole.tera.managers.LayoutManager.LAYOUT_REFLOW_MODE_ALL);
   ...
   function hideWidget(widgetName, reflow)
   {
      _root.HideUI(widgetName,!reflow?0:1);
   }
   function invalidate()
   {
   ...
   {
      var _loc2_ = 0;
      if(this.autoLayoutChanged)
      {
         this.autoLayoutChanged = false;
         while(this.requestedAutoLayoutModes.length > 0)
         {
            com.bluehole.tera.control.LayoutActor.reflowAutoPositionActors(Number(this.requestedAutoLayoutModes.shift()));
         }
      }
   }
   function initLayers()
