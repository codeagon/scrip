class g4.view.premiumWindow.BenefitList extends gfx.core.UIComponent
{
   var highlightShowCompleted = false;
   var highlightSendCompleted = false;
   function BenefitList()
   {
      super();
   }
   function configUI()
   {
      this.itemArray = [];
      var _loc3_ = undefined;
      var _loc4_ = g4.view.premiumWindow.PremiumWindowConstants.BENEFIT_ITEM_MAX_COUNT;
      _loc3_ = 0;
      while(_loc3_ < _loc4_)
      {
         var _loc2_ = this["mc_item_" + _loc3_];
         this.configItem(_loc2_);
         this.cleanUpItem(_loc2_);
         this.itemArray.push(_loc2_);
         _loc3_ = _loc3_ + 1;
      }
   }
   function configItem(mc_item)
   {
      mc_item.mc_slot.drag = true;
      mc_item.mc_slot.SLOT.onPressAux = gfx.utils.Delegate.create(this,function()
      {
         g4.model.GameInterface.callGameInterface("ToGame_BenefitSlotRightClick",String(mc_item.id));
      }
      );
      g4.util.UIUtil.setMouseEnabled(mc_item.mc_highlight,false);
   }
   function cleanUpItem(mc_item)
   {
      mc_item._visible = false;
      mc_item.id = -1;
      mc_item.mc_slot.clear();
      this.setItemHighlight(mc_item,false);
   }
   function setItemHighlight(mc_item, bHighlight)
   {
      if(bHighlight)
      {
         mc_item.mc_highlight._visible = true;
         mc_item.mc_highlight.gotoAndPlay(1);
      }
      else
      {
         mc_item.mc_highlight._visible = false;
         mc_item.mc_highlight.gotoAndStop(1);
      }
   }
   function cleanUpAllHighlights()
   {
      this.highlightShowCompleted = false;
      this.highlightSendCompleted = false;
      this.hideAllHighlights();
   }
   function hideAllHighlights()
   {
      var _loc2_ = undefined;
      var _loc4_ = g4.view.premiumWindow.PremiumWindowConstants.BENEFIT_ITEM_MAX_COUNT;
      _loc2_ = 0;
      while(_loc2_ < _loc4_)
      {
         var _loc3_ = this.getItemAt(_loc2_);
         this.setItemHighlight(_loc3_,false);
         _loc2_ = _loc2_ + 1;
      }
   }
   function cleanUpAllItems()
   {
      var _loc2_ = undefined;
      var _loc3_ = g4.view.premiumWindow.PremiumWindowConstants.BENEFIT_ITEM_MAX_COUNT;
      _loc2_ = 0;
      while(_loc2_ < _loc3_)
      {
         this.cleanUpItem(this.getItemAt(_loc2_));
         _loc2_ = _loc2_ + 1;
      }
   }
   function setData(benefitListData)
   {
      this.cleanUpAllItems();
      var _loc4_ = undefined;
      var _loc6_ = Math.min(benefitListData.getDataCount(),g4.view.premiumWindow.PremiumWindowConstants.BENEFIT_ITEM_MAX_COUNT);
      _loc4_ = 0;
      while(_loc4_ < _loc6_)
      {
         var _loc2_ = benefitListData.getDataAt(_loc4_);
         var _loc5_ = this.getItemAt(_loc4_);
         _loc5_.id = _loc2_.id;
         var _loc3_ = _loc5_.mc_slot;
         _loc3_.upText = _loc2_.name;
         _loc3_.draw(_loc2_.image);
         _loc3_.num_txt.text = String(_loc2_.count);
         _loc3_.impossible = _loc2_.impossible;
         _loc3_.$impossible = _loc2_.impossible;
         _loc3_.grade = _loc2_.grade;
         g4.util.UIUtil.setDragSlotValue(_loc3_,g4.view.ViewID.PREMIUM_WINDOW,_loc2_.id);
         lib.manager.ToolTip.add(_loc3_.SLOT,"1\t" + _loc2_.id,lib.manager.ToolTip.TOOL_TIP_TYPE_PREMIUM_WINDOW,_loc3_.Icon);
         _loc5_._visible = true;
         _loc4_ = _loc4_ + 1;
      }
   }
   function getItemAt(index)
   {
      if(index < 0 || index > g4.view.premiumWindow.PremiumWindowConstants.BENEFIT_ITEM_MAX_COUNT - 1)
      {
         return null;
      }
      return this.itemArray[index];
   }
   function findItemByID(id)
   {
      var _loc2_ = undefined;
      var _loc4_ = g4.view.premiumWindow.PremiumWindowConstants.BENEFIT_ITEM_MAX_COUNT;
      _loc2_ = 0;
      while(_loc2_ < _loc4_)
      {
         var _loc3_ = this.itemArray[_loc2_];
         if(_loc3_.id == id)
         {
            return _loc3_;
         }
         _loc2_ = _loc2_ + 1;
      }
      return null;
   }
   function setToggle(id, registered)
   {
      var _loc2_ = this.findItemByID(id);
      if(_loc2_ == null)
      {
         return undefined;
      }
      _loc2_.mc_slot.active = registered;
   }
   function setCoolTime(id, remainingTime, totalTime)
   {
      var mc_item = this.findItemByID(id);
      if(mc_item == null)
      {
         return undefined;
      }
      var mc_slot = mc_item.mc_slot;
      var slotMc = mc_slot.Slot;
      this.endCoolTime(mc_item);
      if(remainingTime <= 0 || totalTime <= 0)
      {
         return undefined;
      }
      slotMc.cool = new lib.util.CoolTime(remainingTime,totalTime);
      slotMc.cool.onTick = gfx.utils.Delegate.create(this,function()
      {
         if(mc_slot.impossible)
         {
            mc_slot.impossible = false;
         }
         slotMc.Count.txt.text = slotMc.cool.getCountText();
         var _loc1_ = 360 - slotMc.cool.getDgree();
         if(_loc1_ >= 360)
         {
            _loc1_ = 359;
         }
         lib.util.CoolTime.draw(slotMc.CoolTime,50,50,_loc1_,0,60,true);
      }
      );
      slotMc.cool.onFinished = gfx.utils.Delegate.create(this,function()
      {
         this.endCoolTime(mc_item);
      }
      );
      slotMc.cool.updateTick(getTimer());
   }
   function endCoolTime(mc_item)
   {
      var _loc2_ = mc_item.mc_slot;
      var _loc1_ = _loc2_.Slot;
      if(_loc1_.cool != null)
      {
         _loc1_.cool.remove();
         _loc1_.cool.onTick = null;
         _loc1_.cool.onFinished = null;
         delete register1.cool;
      }
      _loc1_.CoolTime.clear();
      _loc1_.Count.txt.text = "";
      _loc2_.impossible = _loc2_.$impossible;
   }
   function show(benefitListData)
   {
      this._visible = true;
      this.showHighlightEffects(benefitListData);
      this.sendHighlightCompleteIDList(benefitListData);
   }
   function hide()
   {
      this._visible = false;
      this.hideAllHighlights();
   }
   function showHighlightEffects(benefitListData)
   {
      if(this.highlightShowCompleted)
      {
         return undefined;
      }
      this.highlightShowCompleted = true;
      if(!benefitListData.hasHighlightIDList())
      {
         return undefined;
      }
      var _loc3_ = undefined;
      var _loc5_ = g4.view.premiumWindow.PremiumWindowConstants.BENEFIT_ITEM_MAX_COUNT;
      _loc3_ = 0;
      while(_loc3_ < _loc5_)
      {
         var _loc2_ = this.getItemAt(_loc3_);
         var _loc4_ = _loc2_.id;
         if(benefitListData.isHighlightID(_loc4_))
         {
            this.setItemHighlight(_loc2_,true);
         }
         else
         {
            this.setItemHighlight(_loc2_,false);
         }
         _loc3_ = _loc3_ + 1;
      }
   }
   function sendHighlightCompleteIDList(benefitListData)
   {
      if(this.highlightSendCompleted)
      {
         return undefined;
      }
      this.highlightSendCompleted = true;
      if(!benefitListData.hasHighlightIDList())
      {
         return undefined;
      }
      this.ToGame_PremiumWindow_SetHighlightCompleteIDList(benefitListData.getHighlightIDList());
   }
   function ToGame_PremiumWindow_SetHighlightCompleteIDList(idList)
   {
      g4.model.GameInterface.callGameInterface("ToGame_PremiumWindow_SetHighlightCompleteIDList",idList);
   }
}
