class lib.gamedata.CustomizedUIDataCollection extends lib.gamedata.GameDataCollectionImpl
{
   ...
   {
      var _loc6_ = data.id == undefined?data.getId():data.id;
      if(this.getData(_loc6_) == null)
      {
         this.addData((lib.gamedata.GameData)data);
         this.dispatchEvent({type:gfx.events.EventTypes.VALUE_CHANGE,target:data,key:_loc6_,value:data.value});
         return true;
      }
      var _loc7_ = this.getIterator();
      while(_loc7_.hasNext())
      {
         var _loc3_ = _loc7_.next();
         var _loc4_ = _loc3_.getId();
         var _loc5_ = _loc3_.getValue();
         if(_loc4_ == _loc6_)
         {
            _loc3_.updateData(data);
            if(_loc5_ != data.value)
            {
               this.dispatchEvent({type:gfx.events.EventTypes.VALUE_CHANGE,target:data,key:_loc6_,value:data.value});
            }
            return true;
